 ...
 * <dd>ON - Indicates that selection listeners are notified continuously during thumb drag events.
 * Otherwise, notification occurs only after the drag event terminates.</dd>
 * <dd>HIGH - Indicates high quality tick marks are generated dynamically to a factor of the
 * pageIncrement or increment. Otherwise, tick marks divide the scale evenly into ten parts.</dd>
 * <dd>SMOOTH - Indicates mouse manipulation of upper and lower values are computed smoothly from the exact
 * mouse cursor position disregarding the increment value.
 * Otherwise, values are constrained to an incremental value.</dd>
 * <dt><b>Events:</b></dt>
 * <dd>Selection</dd>
 * </dl>
 * </p>
...
public class RangeSlider extends Canvas {
	 ...
	 * if the current size is less than 40, no tick marks are produced.
	 */
	protected void setTickFactors() {
		int breadth = orientation == SWT.HORIZONTAL ? getClientArea().width : getClientArea().height;
		if (breadth > 40) {
	 ...
	 * we generate at least ten divisions, but fewer if the ticks are too compact.
	 * some factor of pageIncrement is the preferred tick division.
	 * if too few ticks are generated from pageIncrement then increment is used.
	 * if too few ticks are generated from increment some smaller value is used if style is SMOOTH.
	 */
	protected void setTickFactors(float breadth) {
		int fullRange = maximum - minimum;
		...
		if (tickDivisions > 0) {
			...
			float tickRange = breadth * range / fullRange;
			...
			if (range != fullRange) ++tickDivisions;
		}
	}
	@Override
	public int getStyle() {
		return super.getStyle() | orientation
			| (isSmooth ? SWT.SMOOTH : SWT.NONE)
	...
	/**
	 * set the upperHover and lowerHover values according to the coordinates of the input event,
	 * the key modifier state, and whether the style allows selection of both knobs.
	 * @param e
	 */
	private void selectKnobs(final Event e) {
		...
		upperHover = ((e.stateMask & (SWT.CTRL|SWT.SHIFT)) != 0 || !lowerHover)
						&& x >= coordUpper.x && x <= coordUpper.x + img.getBounds().width && y >= coordUpper.y && y <= coordUpper.y + img.getBounds().height;
	...
	private void handleMouseDoubleClick(final Event e) {
		int value = getCursorValue(e.x, e.y);
		if (value >= 0) {
			...
			if (value > upperValue) {
				if (selectedElement == BOTH) {
					...
					int diff = upperValue - lowerValue;
	 ...
	 * either one or two values are accepted.
	 * @param values
	 */
	...
	private void handleMouseWheel(final Event e) {
		...
		previousLowerValue = lowerValue;
		previousUpperValue = upperValue;
		int amount = increment * ((e.stateMask & SWT.SHIFT) != 0 ? 10 : (e.stateMask & SWT.CTRL) != 0 ? 2 : 1);
		if (selectedElement == BOTH) {
...
// TODO consider API for setting accelerator values
		...
		case SWT.ARROW_LEFT:
			...
			break;
		case SWT.TAB:
			boolean next = (event.stateMask & SWT.SHIFT) == 0;
	...
	/**
	 * translate both the upper and lower values by the input amount. The updated values are constrained to be within
	 * the minimum and maximum. The difference between upper and lower values is retained.
	 * @param amount
	 */
	...
	/**
	 * Sets the 'lower selection', which is the receiver's lower value, to the input argument
	 * which must be less than or equal to the current 'upper selection' and greater or equal to the minimum.
	...
	/**
	 * Sets the minimum and maximum values that the receiver will allow. The new values will
	 * be ignored if either are negative or the min value is not less than the max.
	 * The receiver's selection values will be adjusted if necessary to fall within the new
	 * range.
	 *
	...
	/**
	 * Sets the 'selection', which is the receiver's value. The lower value must be less than or
	 * equal to the upper value. Additionally, both values must be inclusively between the slider
	 * minimum and maximum. If either condition fails, no action is taken.
	 *
	...
	/**
	 * Sets the 'upper selection', which is the upper receiver's value, to the input argument
	 * which must be greater than or equal to the current 'lower selection' and less or equal to the maximum.
	 * If either condition fails, no action is taken.
	 *
